...
BIN22 = '2 2'
BIN11 = '1 1'
ZEROFN = 'Zero_2x2.fits'
ZEROFN1 = 'Zero_1x1.fits'

### Bias & Trim sections from Phil Massey (LMI manual) based on by-eye
###   examination of flatfield images.  The values in the FITS headers
###   for LMI data come from Peter Collins (LOIS author) who computed what
###   he felt were the technically correct values for trim and overscan.
BIASSEC='[3100:3124, 3:3079]'    # Header: BIASSEC = '[3099:3124, 3:3078]'
TRIMSEC='[30:3094,   3:3079]'    # Header: TRIMSEC = '[27:3095,   3:3078]'
...
files = ccdp.ImageFileCollection('.', glob_include='lmi.*.fits')
...
print(files.summary['file', 'imagetyp', 'filter1', 'filter2',
...
for ccd, file_name in files.ccds(ccdsum=BIN22, bitpix=16, imagetyp='bias',
                                 return_fname=True):
...
if len(t_bias_cl.files) != 0:
    trimmed_biases = t_bias_cl.files_filtered(ccdsum=BIN22, imagetyp='bias',
                                              include_path=True)
    
    print("Next, combining 2x2-binned bias frames...")
    ...
    print("Done creating master 2x2-binned bias image.")
...
###  other framses
print("Trim and bias-subtract all other frames (just 2x2 right now)...")

# Reload the list of raw data frames
files = ccdp.ImageFileCollection('.', glob_include='lmi.*.fits')
for ccd, file_name in files.ccds(ccdsum=BIN22, bitpix=16,
                                 return_fname=True):
    ...
    # Write a new header keyword for the combined filter
    ccd.header.append(('FILTCOMB',
                       ccd.header['filter1']+'-'+ccd.header['filter2'],
                       'Combined Filter1-Filter2'), end=True)
...
print("Done bias-subtracting and trimming 2x2-bined images.")
...
for ccd, file_name in bsub_cl.ccds(ccdsum=BIN22, imagetyp='sky flat',
                                    return_fname=True):
...
for ccd, file_name in bsub_cl.ccds(ccdsum=BIN22, imagetyp='dome flat',
                                    return_fname=True):
...
for filter in list(flats_cl.summary['filtcomb']):
    ...
    print(filter)
    flat_fn = (flats_cl.files_filtered(ccdsum=BIN22, filtcomb=filter))[0]
    print(f'Master Flat: {flat_fn}')
    master_flat = CCDData.read(flat_fn)
    
    for ccd, file_name in science_cl.ccds(ccdsum=BIN22, filtcomb=filter,
                                          return_fname=True):
