...
def twodspec_apextract(
    file_name, stype="model", write_1d=True, del_input=False, return_hdr=False
):
    ...
    if stype == "star":
        ...
        spectrum = ccdp.block_average(ccd, [n_y, 1])  # Squash, keeping header
        ...
        for i in range(n_x):
            spectrum.data[0, i] = np.average(
                ccd.data[int(trace[i]) - halfwin : int(trace[i]) + halfwin + 1, i]
            )
    ...
    pixnum = np.asarray(range(n_x)).flatten()
...
def twodspec_apdefine(ccd, stype="star"):
    ...
    if stype == "star":
        ...
        trace = np.full(n_x, n_y / 2, dtype=float)
        width = 201
